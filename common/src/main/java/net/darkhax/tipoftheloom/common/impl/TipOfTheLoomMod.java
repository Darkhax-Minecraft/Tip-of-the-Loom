...
public class TipOfTheLoomMod {
    ...
    public static void getPatternTooltips(Holder<BannerPattern> pattern, boolean isSelected, DyeColor dye, List<Component> tooltip, TooltipFlag flags) {
        if (CONFIG.enabled) {
            // Display Name
            tooltip.add(Component.translatable(pattern.value().translationKey() + "." + dye.getName()));
            pattern.unwrapKey().ifPresent(key -> {
                ...
                // Mod Name
                if (CONFIG.mod_name.enabled && (CONFIG.mod_name.display_on_vanilla_patterns || !Identifier.DEFAULT_NAMESPACE.equals(key.identifier().getNamespace()))) {
